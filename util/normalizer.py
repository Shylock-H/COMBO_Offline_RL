...
class StandardNormalizer(object):
    ...
    def fit(self, data):
        ...
        if isinstance(data, torch.Tensor):
            self.mean = torch.mean(data, dim=0, keepdims=True).to(DEVICE)
            self.var = torch.var(data, dim=0, keepdims=True).to(DEVICE)
    ...
    def update(self, samples):
        ...
        if self.tot_count == 0:
            ...
            if isinstance(samples, torch.Tensor):
                self.mean = torch.zeros(dim, dtype=torch.float32).to(DEVICE)
                self.var = torch.ones(dim, dtype=torch.float32).to(DEVICE)
