...
def convert_tfenvents_to_csv(root_dir, xlabel, ylabel):
    ...
    with tqdm.tqdm(tfevent_files) as t:
        for tfevent_file in t:
            ...
            for test_rew in ea.scalars.Items('performance/' + ylabel):
                content.append(
                    [
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--task', default='halfcheetah-medium-replay-v2', help='task'
    )
    parser.add_argument(
        '--algos', default=["COMBO", "COMBO-tf"], help='algos'
    )
    parser.add_argument(
        '--title', default=None, help='matplotlib figure title (default: None)'
    )
    parser.add_argument(
        '--xlabel', default='epochs', help='matplotlib figure xlabel'
    )
    parser.add_argument(
        '--ylabel', default='eval_return', help='matplotlib figure ylabel'
    )
    parser.add_argument(
        '--smooth', type=int, default=1, help='smooth radius of y axis (default: 0)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--output-path', type=str, help='figure save path', default="./result/"
    )
    parser.add_argument(
        '--dpi', type=int, default=200, help='figure dpi (default: 200)'
    )
    args = parser.parse_args()
    args.title = args.task
    args.output_path = args.output_path + args.task + '.png'
